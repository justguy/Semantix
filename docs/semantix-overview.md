...
It does not have to exist in only one client form. The same control room can be rendered as:

- a browser-based control surface
- a terminal-native control surface or TUI such as `stx`

What matters is not the chrome. What matters is that both clients consume the same structured
review artifact and the same backend approval semantics.
...
In a browser, that may appear as synchronized panes. In a terminal-native client, it may appear as
stacked sections, focus views, or a split-pane TUI. The interaction model should stay the same even
