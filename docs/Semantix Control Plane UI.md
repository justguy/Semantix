...
It may be rendered in more than one client form. The two first-class forms are:

- a browser-based control surface
- a terminal-native control surface or TUI, for example `stx`

Both clients should consume the same Semantix review and control contract through the same backend
source of truth.
...
One available execution runtime may be Phalanx for software engineering. Phase-1 prototypes may
also use a runtime adapter over Codex CLI. Later versions can add other runtimes, generic
workflows, and runtime adapters under the same surface.

### 2.2 Client Forms

The Control Surface contract is client-neutral.

It should support at least:

- web UI rendering for richer spatial layouts
- terminal-native rendering for CLI or TUI workflows

The browser client and terminal-native client are peers. Neither should bypass the control plane or
derive authoritative state from runtime transcripts.

### 2.3 Local Browser Host

The phase-1 browser control surface is built and hosted by `packages/stx`.

- `npm run build:ui` emits the browser assets into `packages/stx/dist/ui`
- `stx serve` builds that UI locally, serves `/index.html` plus bundled assets, and exposes the
  Semantix HTTP API from the same process
- legacy browser prototype routes may redirect to `/index.html`, but the canonical browser entry
  path is `/`

This keeps the browser and terminal-native `stx` as peer clients over one local Semantix control
plane rather than two different host processes.
...
The backend control plane remains the source of truth for review artifacts, approvals, freshness,
and execution coordination. It may in turn coordinate one or more runtimes such as Phalanx or
Codex CLI through runtime adapters. The Control Surface is a versioned projection of backend state.

The same rule applies to a terminal-native client. It is also a versioned projection of backend
state, not a direct authority over execution.
...
For terminal-native clients, the same artifact may be rendered as:

- a split-pane TUI
- a stacked scroll view with focused inspectors
- command-targeted views such as `graph`, `inspect`, and `diff`

The presentation can vary by client. The semantics cannot.
...
The backend control plane is the source of truth for review artifacts, freshness, and approval
state. It may coordinate one or more runtimes underneath it, including Phalanx or Codex CLI
through runtime adapters.
