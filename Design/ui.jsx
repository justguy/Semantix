...
function normalizeProposedChange(artifact, change) {
  ...
  return {
    ...change,
    nodeId,
    node: change.node || nodeId,
    originatingNodeId: change.originatingNodeId || nodeId,
    preview,
  };
}

function decorateArtifact(rawArtifact) {
  ...
  artifact.proposedChanges = existingChanges.map((change) => normalizeProposedChange(artifact, change));
  return artifact;
}

function normalizeInspectorPayload(artifact, rawPayload) {
  ...
  if (Array.isArray(payload.proposedChanges)) {
    payload.proposedChanges = payload.proposedChanges.map((change) =>
      normalizeProposedChange(artifact, change),
    );
  }
...
function hydrateArtifactWithInspectorCache(rawArtifact, inspectorCache) {
  ...
  getNodes(artifact).forEach((node) => {
    ...
    artifact.nodeInspectorPayloads[node.id] = normalizeInspectorPayload(artifact, cachedPayload);
  });

  Object.entries(artifact.nodeInspectorPayloads).forEach(([nodeId, payload]) => {
    artifact.nodeInspectorPayloads[nodeId] = normalizeInspectorPayload(artifact, payload);
  });

  return artifact;
}

function buildStaleArtifactView(currentArtifact, latestArtifact, focusNodeId) {
  if (!currentArtifact || !latestArtifact) {
    return decorateArtifact(currentArtifact);
  }

  const staleArtifact = decorateArtifact(currentArtifact);
